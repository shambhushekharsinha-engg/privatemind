...
export default function Dashboard() {
  ...
  const [encryptionKey, setEncryptionKey] = useState(null); // Keeps the plain password string ONLY in volatile memory
  ...
  const loadDatabaseData = async (masterPassword = encryptionKey) => {
    if (!masterPassword) return; // Halt if the cryptographic vault is locked
    try {
      const allFolders = await folderDB.getAll();
      const allNotes = await noteDB.getAll();

      // Dynamically map through notes and decrypt any encrypted contents in memory
      const decryptedNotes = await Promise.all(
        allNotes.map(async (note) => {
          ...
          return note; // Return as-is if it's plaintext legacy data
        })
      );
  ...
  // 2. Trigger Model Ingestion Event (Kept pristine & decoupled)
  ...
  // 3. Dispatch Interaction Query Context (Sends decrypted volatile text safely to the AI Worker)
  const handleSendQuery = () => {
    ...
    // Because activeNote contains the decrypted plain text string in the React state memory,
    // we safely package it up without ever exposing the encrypted database ciphertext blocks to the AI thread!
    let fullPrompt = "";
    if (activeNote) {
  ...
  const handleNoteContentChange = async (content) => {
    ...
    // 1. Instantly update the active note in React memory so typing stays fluid and real-time
    setActiveNote((prev) => ({ ...prev, content }));

    try {
      // 2. Encrypt the raw string using the master password stored in volatile memory
      const cipherText = await encryptData(content, encryptionKey);
      const securePayload = "ENCRYPTED:" + cipherText;

      // 3. Save the encrypted string safely to IndexedDB
      await noteDB.update(activeNote.id, { content: securePayload });

      // 4. Pull all notes from the database and decrypt them for the sidebar list state
      const allNotes = await noteDB.getAll();
      const decryptedNotes = await Promise.all(
        allNotes.map(async (note) => {
          if (note.content && note.content.startsWith("ENCRYPTED:")) {
  ...
  const handleFactoryReset = async () => {
    ...
    try {
      // 1. Wipe out volatile memory states immediately
      ...
      // 2. Clear out the localStorage password hash reference
      localStorage.removeItem('pm_vault_hash');

      // 3. Clear the Dexie IndexedDB tables fully
      await folderDB.clear();
      await noteDB.clear();

      alert("Vault completely wiped. The application will now restart in fresh initialization mode.");
      window.location.reload(); // Hard refresh to reset the complete application lifecycle
    } catch (error) {
  ...
  return (
    <>
      ...
      <div className={`flex h-screen w-screen overflow-hidden bg-zinc-950 text-zinc-100 antialiased transition-all duration-300 ${isLocked ? 'blur-md pointer-events-none scale-98 select-none' : ''}`}>
        ...
        <main className="flex flex-1 flex-col bg-zinc-950">
          <header className="flex h-14 items-center justify-between border-b border-zinc-800 px-6">
            ...
            <div className="flex items-center gap-3">
              {/* 🔒 MANUAL LOCK BUTTON */}
              <button 
                onClick={() => { setEncryptionKey(null); setIsLocked(true); }}
